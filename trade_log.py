...
def _write_header():
    ...
    with open(TRADES_LOG, "w", encoding="utf-8") as f:
        ...
        hdr = (
            ...
            f"{'Outcome':<18} {'Result':<8} {'SL':<4} {'Net $':<12} "
...
def log_trade_event(
    *,
    ...
    if skipped:
        ...
        result_s  = "LOSS"
        outcome_s = outcome or "?"
        net_s     = f"{net:+.4f}$" if net is not None else f"-${TRADE_AMOUNT:.2f}"
        row_note  = f"  | {note}" if note else ""

    row = (
        f"{n:<5} {ts:<22} {slug[:37]:<38} {side[:5]:<6} "
        f"${price:<7.4f} {str(shares):<8} ${str(payout):<9} "
        f"{outcome_s[:17]:<18} {result_s:<8} {sl_s:<4} {net_s:<12} "
...
def build_summary(state: dict) -> str:
    ...
    wins     = [t for t in resolved if t["won"] and not t.get("sl_hit")]
    losses   = [t for t in resolved if not t["won"] and not t.get("sl_hit")]
    early    = [t for t in all_t if t.get("sold_early")]
    sl_hits  = [t for t in all_t if t.get("sl_hit")]
    ...
    lines.append(f"  SL hits             : {len(sl_hits)}")
    ...
    lines.append("")
    lines.append(
        f"  {'#':<4} {'Slug':<38} {'Side':<6} {'Price':<8} {'Outcome':<18} "
        f"{'Result':<8} {'SL':<4} {'Net':<12} Link"
    )
    lines.append("  " + "-" * 128)
    ...
    for i, (slug, t) in enumerate(trades.items(), 1):
        ...
        sl_s    = "YES" if t.get("sl_hit") else "NO"
        ...
        lines.append(
            ...
            f"{outcome[:17]:<18} {result_s:<8} {sl_s:<4} {net_s:<12} {url}"
        )
